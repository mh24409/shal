...
use App\Http\Controllers\FacebookCatalog;
...
//     for($i = 0 ; $i < 500 ; $i++)
//     {
//         $translate = Translation::where('lang','!=','eg')->Where('lang','!=','en')->paginate(500);
...
Route::get('cache',function (){
   ...
   dd(get_setting('shipping_profit'));
   Artisan::call('optimize:clear'); 
});

Route::get('/catalog/{lang}',function (Request $request){
    ...
    foreach ($products as $product) {
        ...
        if ($product->brand_id == null) {
            ...
            $sale_price = home_discounted_base_price_catalog($product);
        ...
        $id = $product->id;
        $name =  $product->getTranslation('name',$request->lang);
        $description = htmlSpecialChars($product->getTranslation('short_description',$request->lang));
        $price = home_base_price_catalog($product);
        $categories = Category::find($product->category_id)->name;
        $content .=
    "<item>
    <g:id>$id</g:id>
    <g:title>$name</g:title>
    <g:description>$description</g:description>
    <g:availability>$is_available</g:availability>
    <g:condition>new</g:condition>
    <g:price>$price</g:price>
    <g:link>$url</g:link>
    <g:image_link>$img_link</g:image_link>
    ...
    Route::get('/aymakan_cities', function () {
        $aymakan_cities = DB::select('select * from aymakan_cities');
        $perPage = 8;
        $currentPage = LengthAwarePaginator::resolveCurrentPage();
        $currentItems = array_slice($aymakan_cities, ($currentPage - 1) * $perPage, $perPage);
        $cities = new LengthAwarePaginator($currentItems, count($aymakan_cities), $perPage, $currentPage);
        return view('backend.setup_configurations.cities.aymakan.aymakan_cities', compact('cities'));
    })->name('aymakan_city_attach');
    Route::get('/attach_aymakan_cities/{id}', function ($id) {
        $state = State::find($id);
        $aymakan_cities = DB::select('select * from aymakan_cities');
        $state_cities = DB::table('aymakan_cities')->where('city_id', $state->id)->pluck('id')->toArray();;
        return view('backend.setup_configurations.cities.aymakan.attach_cities', compact('state', 'state_cities', 'aymakan_cities'));
    })->name('aymakan_city_attach.attach');


    Route::post('/save_attached_aymakan_cities', function (Request $request) {
        foreach ($request->cities as $city_id) {
            $city = DB::table('aymakan_cities')->where('id', $city_id)->first();
            if ($city) {
                $city->city_id = $request->state_id;
                DB::table('aymakan_cities')->where('id', $city_id)->update(['city_id' => $request->state_id]);
            }
        }
        return redirect()->route('aymakan_city_attach');
    })->name('aymakan_city_attach.store');



    Route::get('/jana_cities', function () {
        $jana_cities = DB::select('select * from jana_cities');
        $perPage = 8;
        $currentPage = LengthAwarePaginator::resolveCurrentPage();
        $currentItems = array_slice($jana_cities, ($currentPage - 1) * $perPage, $perPage);
        $cities = new LengthAwarePaginator($currentItems, count($jana_cities), $perPage, $currentPage);
        return view('backend.setup_configurations.cities.jana.jana_cities', compact('cities'));
    })->name('jana_city_attach');
    Route::get('/attach_jana_cities/{id}', function ($id) {
        $state = State::find($id);
        $jana_cities = DB::select('select * from jana_cities');
        $state_cities = DB::table('jana_cities')->where('city_id', $state->id)->pluck('id')->toArray();;
        return view('backend.setup_configurations.cities.jana.attach_cities', compact('state', 'state_cities', 'jana_cities'));
    })->name('jana_city_attach.attach');

    Route::post('/save_attached_janacities', function (Request $request) {
        foreach ($request->cities as $city_id) {
            $city = DB::table('jana_cities')->where('id', $city_id)->first();
            if ($city) {
                $city->city_id = $request->state_id;
                DB::table('jana_cities')->where('id', $city_id)->update(['city_id' => $request->state_id]);
            }
        }
        return redirect()->route('jana_city_attach');
    })->name('jana_city_attach.store');
    Route::post('/save_jana_city', function (Request $request) {
        // return $request ;
        DB::table('jana_cities')->insert([
            'city_id' => 1,
            'name_ar' => $request->name_ar,
            'name_en' => $request->name_en,
            'shipping_id' => $request->state_id
        ]);
        flash(translate('Address info updated successfully'))->success();

        return redirect()->route('jana_city_attach');
    })->name('save_jana_city');
    Route::get('/delete_jana_city/{id}', function ($city_id) {
        DB::table('jana_cities')->where('id', $city_id)->delete();
        flash(translate('Address info updated successfully'))->success();
        return redirect()->route('jana_city_attach');
    })->name('delete_jana_city');
    ...
    Route::resource('questions_answers', QuestionAnswerController::class);
    Route::controller(QuestionAnswerController::class)->group(function () {
        Route::get('/questions_answers/edit/{id}', 'edit')->name('questions_answers.edit');
        Route::get('/questions_answers/destroy/{id}', 'destroy')->name('questions_answers.destroy');
    });
    ...
    Route::controller(ProductController::class)->group(function () {
        ...
        Route::get('/products/create/barcode', 'create_barcode')->name('products.create_barcode');
        Route::post('/get-product-stock}', [ProductController::class, 'getProductStock'])->name('getProductStock');
        Route::post('/products/trending', 'updateTrending')->name('products.trending');
        Route::post('/products/best-selling', 'updateBestSelling')->name('products.best_selling');
        Route::get('/best-selling/products/indexes','getBestSelling')->name('products.best_selling_index');
        Route::put('/best-selling/products/indexes/{product}','updateProductBestSellingIndex')->name('products.best_selling_index.update');
    ...
    Route::controller(BusinessSettingsController::class)->group(function () {
        ...
        Route::post('/facebook_conversation', 'facebook_conversation_update')->name('facebook_conversation.update');
    ...
    Route::get('/order/reciept/{id}', [OrderController::class, 'reciept'])->name('products.reciept');
    Route::controller(OrderController::class)->group(function () {
        Route::get('/update_shipping_status/{order_id}', 'update_shipping_status')->name('all_orders.update_shipping_status');
        Route::get('/FizpaNewOrder', 'fizpa')->name('FizpaNewOrder.index');
        Route::post('/order/notes', 'add_notes')->name('add.notes');
        ...
        Route::get('/NewOrder', 'new_order_by_admin')->name('new_order_by_admin');
        Route::post('/AddOrder', 'add_order_by_admin')->name('add_order_by_admin');

        Route::middleware('orderEditAccess')->group(function (){
            Route::get('/EditOrder/{order}', 'edit_order_by_admin')->name('edit_order_by_admin');
            Route::put('/UpdateOrder/{order}', 'update_order_by_admin')->name('update_order_by_admin');
            // Route::get('/generate-shipping-info-pdf/{order}','otherShippingCompanyAddressPdf')->name('print_order_other_shipping_company_info');
        });
    ...
    Route::controller(ReportController::class)->group(function () {
        Route::get('/reports', 'reports')->name('reports.home');
        Route::get('/category/reports', 'category_reports')->name('category_reports.home');
        Route::get('/products/reports', 'products_reports')->name('products_reports.home');
        Route::get('/reports/showproduct/{id}/{date?}','showproduct')->name('reports.showproduct');
        Route::get('/reports/views/{date?}', 'pagesView')->name('reports.pagesview');
        Route::get('/order/reports', 'order_reports')->name('order_reports.home');


        Route::get('/in_house_sale_report', 'in_house_sale_report')->name('in_house_sale_report.index');
        Route::get('/profitReport', 'profitReport')->name('profitReport.index');
    ...
    Route::resource('catalog', FacebookCatalog::class);
    Route::get('delete_catalog/{id}',[FacebookCatalog::class,'destroy'])->name('delete_catalog');
...
//Reviews
    Route::controller(ReviewController::class)->group(function () {
        ...
        Route::get('/add-custom-reviews', 'createCustomReviews')->name('reviews.create_custom');
        Route::get('/reviews/destroy/{id}','destroy')->name('reviews.destroy.by_admin');
        Route::post('/custom-review','customProductReview')->name('reviews.custom_review');
        Route::post('/remove-custom-review','removeCustomReviews')->name('reviews.remove_custom_review');
	...
	Route::controller(StateController::class)->group(function (){
        Route::get('shipping-company/states','updateShippingCompanyStates')->name('update_shipping_company_states');
	});
    ...
    Route::controller(CityController::class)->group(function () {
        ...
        Route::get('/shipping-cities/store','store_shipping_cities_from_company')->name('shipping_cities.store');
    });
    ...
    Route::controller(AizUploadController::class)->group(function () {
       ...
       Route::post('/save-cropped-image', 'saveCroppedImage')->name('save-cropped-image');
