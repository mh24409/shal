...
class AizUploadController extends Controller
{
    ...
    $croppedData = $request->input('cropped_data');
    ...
    $originalFilePath = public_path($upload->file_name);

    // Ensure that the cropped data contains integer values
    $croppedData = array_map('intval', $croppedData);

    // Generate a unique filename
    $newFilename = uniqid('cropped_') . '_' . time() . '.' . $upload->extension;

    $image = Image::make($originalFilePath);
    $image->crop($croppedData['width'], $croppedData['height'], $croppedData['x'], $croppedData['y']);
    $image->save(public_path('uploads/all/' . $newFilename));
    ...
    return response()->json(['status' => 'success', 'file_url' => uploaded_asset($upload->id)]);
    }
}
