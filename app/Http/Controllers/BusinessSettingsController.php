...
class BusinessSettingsController extends Controller
{
    ...
    {
        $currentTime = Carbon::now()->addHours($request->count_to);

                $carbonDate = Carbon::parse($request->counter_end);
                $convertedFormat = $currentTime->format('Y-m-d H:i:s');
                $counter_end = BusinessSetting::where('type', 'counter_end')->first();
                if($counter_end ){

                    $counter_end->update([
                        'value'=> $request->count_to,
                        'updated_at'=>$convertedFormat
                    ]);
                }else{
    ...
    {
        $business_settings = BusinessSetting::where('type', $request->type)->first();
        $business_settings->value = $request[$request->type] ?? 0;
        $business_settings->save();
    ...
    public function facebook_conversation_update(Request $request)
    {
        foreach ($request->types as $key => $type) {
                $this->overWriteEnvFile($type, $request[$type]);
        }
        $business_settings = BusinessSetting::firstOrCreate([
            'type'=>'facebook_converasion_api'
        ]);
        if ($request->has('facebook_converasion_api')) {
            $business_settings->value = 1;
            $business_settings->save();
        }
        else{
