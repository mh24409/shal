...
if (!function_exists('AuthOtp')) {
    function AuthOtp($phone, $otpCode, $user)
    { 
        try {
            $client = new Client();
...
if (!function_exists('OrderOtp')) {
    function OrderOtp($phone, $otpCode, $user)
    {
        try {
            $client = new Client();
            $baseUrl = env('EDNA_BASEURL');
    ...
    function filter_products($products)
    {
        ...
        if (get_setting('vendor_system_activation') == 1) {
            return $products->where('approved', '1')
                ->where('published', '1')
                ->where('auction_product', 0)->with('stocks')
        ...
        if (auth()->guest()) {
            ...
            if ($coupon != null) {
                ...
                if (strtotime(date('d-m-Y')) >= $coupon->start_date && strtotime(date('d-m-Y')) <= $coupon->end_date || $coupon->lifetime == 1) {
                    if (allowed_to_use_coupon($coupon, $temp_user_id) || CouponUsage::where('temp_user_id', $temp_user_id)->where('coupon_id', $coupon->id)->first() == null) {
            ...
            if ($coupon != null) {
                if (strtotime(date('d-m-Y')) >= $coupon->start_date && strtotime(date('d-m-Y')) <= $coupon->end_date || $coupon->lifetime == 1) {
                    if (allowed_to_use_coupon($coupon, Auth::user()->id) || CouponUsage::where('user_id', Auth::user()->id)->where('coupon_id', $coupon->id)->first() == null) {
                        $coupon_details = json_decode($coupon->details);
...
if (!function_exists('home_discounted_base_price_by_stock_id_admin')) {
    function home_discounted_base_price_by_stock_id_admin($id)
    {
        $product_stock = ProductStock::findOrFail($id);
        $product = $product_stock->product;
        $price = $product_stock->price;
...
function getShippingCost($carts, $index, $carrier = '', $shipping_info = [])
{
    ...
    if ($shipping_type == 'flat_rate') {
        ...
        $city = City::where('id', $shipping_info['city_id'] ?? null)->first();
        if ($city != null) {
            if ($product->added_by == 'admin') {
        ...
        if (auth()->check() && auth()->user()) {
            ...
            $total_of_carts = Cart::where('user_id', auth()->user()->id)->sum('price');
            $count_of_carts = Cart::where('user_id', auth()->user()->id)->count();
            if ($total_of_carts >= get_setting('allwed_free_shipping_discount')  || $count_of_carts >= get_setting('allowed_free_shipping_quantity')) {
                return 0;
            }
            if ($product->added_by == 'admin') {
                return $city->cost / count($admin_products);
        ...
        $user_zone = Country::where('id', $shipping_info['country_id'] ?? null)->zone_id;
...
if (!function_exists('uploaded_asset')) {
    ...
    {
        if (($asset = get_upload_by_id($id)) != null) {
...
if (!function_exists('get_setting')) {
    function get_setting($key = null, $default = null, $lang = false)
    {
        
        if (!Cache::has('business_settings')) {
            Cache::remember('business_settings', 86400, function () {
                $settings = [];
                BusinessSetting::get()->map(function ($setting) use (&$settings) {
                    if ($setting->lang != null) {
                        $name = $setting->type . $setting->lang;
                    } else {
                        $name = $setting->type;
                    }
        ...
        $settings = Cache::get('business_settings');

        if ($lang == false) {
            $setting = $settings[$key] ?? null;
        } else {
            $setting = $settings[$key . $lang] ?? null;
        }
        return $setting == null ? $default : $setting;
    }
}

if (!function_exists('getStockQuantity')) {
    function getStockQuantity($product_id)
    {

        if (!Cache::has('products_stocks_qty')) {
            Cache::remember('products_stocks_qty', 864000, function () {
                $stocks = [];
                Product::with('stocks')->get()->map(function ($product) use (&$stocks) {
                    $stocks[$product->id] = $product->stocks->sum('qty');
                });
                return $stocks;
            });
        }
        $stocks = Cache::get('products_stocks_qty');
        $stock = $stocks[$product_id] ?? 0;
        return  $stock;
    }
}

if (!function_exists('home_base_price_catalog')) {
    function home_base_price_catalog($product, $formatted = true)
    {
        $price = $product->unit_price;
        $tax = 0;
        ...
        $price += $tax;
        return $formatted ? convert_price($price) : $price;
    }
}

if (!function_exists('home_discounted_base_price_catalog')) {
    function home_discounted_base_price_catalog($product, $formatted = true)
    {
        $price = $product->unit_price;
        $tax = 0;
...
if (!function_exists('get_colors_by_code')) {
    function get_colors_by_code($code = null)
    {
        if (!Cache::has('colors')) {
            Cache::remember('colors', 86400, function () {
                $colors = [];
                Color::get()->map(function ($color) use (&$colors) {

                    $code = $color->code;

                    $colors[$code] = $color->name;
                });
                return $colors;
            });
        }
...
if (!function_exists('get_upload_by_id')) {
    function get_upload_by_id($id = null)
    {
        if (!Cache::has('uploads')) {
            Cache::remember('uploads', 86400, function () {
                $uploads = [];
                Upload::get()->map(function ($upload) use (&$uploads) {
...
if (!function_exists('get_product_stock_img')) {
    function get_product_stock_img($product_id = null, $variation = null)
    {
        if (!Cache::has('products_stocks')) {
            Cache::remember('products_stocks', 86400, function () {
                $stocks = [];
                ProductStock::select('product_id', 'variant', 'image')->get()->map(function ($stock) use (&$stocks) {

                    $stocks[$stock->product_id][] = $stock->toArray();
                });
                return $stocks;
            });
        }
        $stocks = Cache::get('products_stocks');

        $image = collect($stocks[$product_id])->where('variant', $variation)->first()->image ?? null;
        return $image;
    }
}
...
// allowed to use coupon
if (!function_exists('allowed_to_use_coupon')) {
    function allowed_to_use_coupon(Coupon $coupon, $user_id)
    {
        $allowed = 0;
        $coupon_is_limit_for_one_user = $coupon->is_user_limit;
        $coupon_limitition_for_one_user = $coupon->users_limit;

        $coupon_is_limit_uses = $coupon->is_limited;
        $coupon_limitition_for_uses = $coupon->uses_limit;


        if ($coupon_is_limit_for_one_user == 1 || $coupon_is_limit_uses == 1) {
            if ($coupon_is_limit_for_one_user == 1) {
                $last_uses_number = 0;
                if (Auth::check()) {
                    $last_uses_number = count(CouponUsage::where('user_id', $user_id)->where('coupon_id', $coupon->id)->get());
                } else {
                    $last_uses_number = count(CouponUsage::where('temp_user_id', $user_id)->where('coupon_id', $coupon->id)->get());
                }
                if ($last_uses_number < $coupon_limitition_for_one_user) {
                    $allowed = 1;
                }
            }
            if ($coupon_is_limit_uses == 1) {
                if ($coupon->uses_limit > $coupon->num_of_uses) {
                    $allowed = 1;
                } else {
                    $allowed = 0;
                }
            }
        } elseif (CouponUsage::where('temp_user_id', $user_id)->orWhere('user_id', $user_id)->where('coupon_id', $coupon->id)->first() == null) {
            $allowed = 1;
        }
        return $allowed;
    }
