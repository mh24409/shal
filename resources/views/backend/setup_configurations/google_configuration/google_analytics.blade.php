    ...
    <div class="row">
        ...
        <div class="col-md-6">
            <div class="card">
                ...
                <div class="card-body">
                    <form class="form-horizontal" action="{{ route('facebook_conversation.update') }}" method="POST">
                        @csrf
                        <div class="form-group row">
                            <div class="col-lg-3">
                                <label class="col-from-label">{{ translate('Facebook Converasion Api') }}</label>
                            </div>
                            <div class="col-md-7">
                                <label class="aiz-switch aiz-switch-success mb-0">
                                    <input value="1" name="facebook_converasion_api" type="checkbox" @if (get_setting('facebook_converasion_api') == 1)
                                        checked
                                    @endif>
                                    <span class="slider round"></span>
                                </label>
                        ...
                        <div class="form-group row">
                            <input type="hidden" name="types[]" value="CONVERSIONS_API_PIXEL_ID">
                            <div class="col-lg-3">
                                <label class="col-from-label">{{ translate('CONVERSIONS_API_PIXEL_ID') }}</label>
                            </div>
                            <div class="col-md-7">
                                <input type="text" class="form-control" name="CONVERSIONS_API_PIXEL_ID" value="{{  env('CONVERSIONS_API_PIXEL_ID') }}" placeholder="{{ translate('CONVERSIONS_API_PIXEL_ID') }}" required>
                            </div>
                        </div>
                        <div class="form-group row">
                            <input type="hidden" name="types[]" value="CONVERSIONS_API_PIXEL_TOKEN">
                            <div class="col-lg-3">
                                <label class="col-from-label">{{ translate('CONVERSIONS_API_TOKEN') }}</label>
                            </div>
                            <div class="col-md-7">
                                <input type="text" class="form-control" name="CONVERSIONS_API_PIXEL_TOKEN" value="{{  env('CONVERSIONS_API_PIXEL_TOKEN') }}" placeholder="{{ translate('CONVERSIONS_API_TOKEN') }}" required>
                            </div>
                        </div>
                        <div class="form-group row">
                            <input type="hidden" name="types[]" value="CONVERSATION_TEST_CODE">
                            <div class="col-lg-3">
                                <label class="col-from-label">{{ translate('CONVERSATION_TEST_CODE') }}</label>
                            </div>
                            <div class="col-md-7">
                                <input type="text" class="form-control" name="CONVERSATION_TEST_CODE" value="{{  env('CONVERSATION_TEST_CODE') }}" placeholder="{{ translate('CONVERSATION_TEST_CODE') }}" required>
                            </div>
                        </div>
                        <div class="form-group mb-0 text-right">
                            <button type="submit" class="btn btn-sm btn-primary">{{translate('Save')}}</button>
