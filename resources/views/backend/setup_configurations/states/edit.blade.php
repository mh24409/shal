            ...
            <div class="card-body p-0">
                <form class="p-4" action="{{ route('states.update', $state->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <select class="select2 form-control aiz-selectpicker" name="state_name" data-toggle="select2" data-placeholder="Choose ..." data-live-search="true">
                            @foreach (\App\Models\Shipping_Governments::get() as $shipping_state)
                                <option value="{{ $shipping_state->name }}_{{$shipping_state->state_id}}" {{$state->shipping_name == $shipping_state->name ? 'selected' : null}}>
                                    {{ $shipping_state->name }}
                                </option>
                            @endforeach
                        </select>
