                            ...
                            <div class="aiz-file-box">
                                <div class="dropdown-file">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-right">
                                        ...
                                        <a href="javascript:void(0)" class="dropdown-item confirm-alert crop-button"
                                            data-target="#image{{ $file->id }}" data-toggle="modal"
                                            data-id="{{ $file->id }}">
                                            <i class="las la-trash mr-2"></i>
                                            <span>{{ translate('Crop and Upload') }}</span>
                            </a>
                                    </div>
                                </div>
                                ...
                                <div class="card card-file aiz-uploader-select c-default"
                                    ...
                                    <div class="card-file-thumb">
                                        @if ($file->type == 'image')
                                            <img src="{{ $file_path }}" class="img-fit" id="image{{ $file->id }}">
                        ...
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                <div class="cropper-container">
                                    <img src="" id="cropImage" class="img-fit">
                                </div>
                            </div>
                            <div class="modal-footer">
                                <button type="button" class="btn btn-secondary close-crop-modal"
                                    data-dismiss="modal">{{ translate('Close') }}</button>
                                <button type="button" class="btn btn-primary"
                                    id="confirmCrop">{{ translate('Confirm Crop') }}</button>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        $(document).ready(function() {
            ...
            $('#cropModal').on('shown.bs.modal', function() {
                ...
                cropper = new Cropper(image[0], {
                    ...
                    autoCropArea: 1, // Set to 1 for content cropping
                });

                // Handle the "Confirm Crop" button click
                $('#confirmCrop').click(function() {
                    var croppedData = cropper.getData();

                    // Use the data-id attribute to get the file ID
                    ...
                    $.ajax({
                        ...
                        data: {
                            ...
                            cropped_data: croppedData,
                            '_token': '{{ csrf_token() }}'
                        },
                        success: function(response) {
                            if (response.status === 'success') {
                                // Optionally, you can perform actions after a successful save

                                var imageUrl = response.file_url;


                                var html = `<img src="${imageUrl}" class="img-fit" id="image${fileId}"  aria-hidden="true">`
                                $('#image' + fileId).replaceWith(html)
                        ...
                        error: function(error) {
                            ...
                            AIZ.plugins.notify('danger', 'Something went wrong ' + error);
                        }
                    });
                ...
                $('#close-crop-modal').click(function (){
            ...
            $('.crop-button').click(function(e) {
                e.preventDefault();
                // Get the image source and set it to the crop modal
                var imageSource = $(this).closest('.aiz-file-box').find('.img-fit').attr('src');
                // Set the data-id attribute with the file ID
                var fileId = $(this).data('id');
                $('#confirmCrop').data('id', fileId);
                $('#cropImage').attr('src', imageSource);
