<form action="{{ route('addresses.update',$address->id) }}" class="row gutters-10  " method="POST">
    @csrf
    <div class="col-6 p-1 ">
    ...
    <div class="col-6 p-1">
        ...
        <input name="phone" dir="ltr" style="text-align:right;" type="phone"  id="phone-code" value="{{ $address->phone }}"
            id="phone_number" required class="rounded w-lg-75 checkout-input  "
            placeholder="5612 345 67">
        @error('phone')
            <span class="text-danger">{{ $message }}</span>
        @enderror
    </div>
    <div class="d-flex col-12 justify-content-end align-items-center px-2 ">
        <button
            class=" btn fs-18 w-100 dark-button-style ">{{ translate('Update Address') }}</button>
    </div>
</form>
