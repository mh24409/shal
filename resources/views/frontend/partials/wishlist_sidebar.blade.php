<div class="aiz-top-menu-sidebar-wishlist collapse-sidebar-wrap sidebar-all sidebar-left z-1035">
    <div class="overlay overlay-fixed dark c-pointer" data-toggle="class-toggle"
        data-target=".aiz-top-menu-sidebar-wishlist" data-same=".hide-top-menu-bar"></div>
    <div style="overflow: visible;"
        ...
        <button type="button" class=" hide-top-menu-bar close-sidebar" data-toggle="class-toggle"
            data-target=".aiz-top-menu-sidebar-wishlist">
        ...
        <div class="h-100 w-100 d-flex justify-content-between align-items-center flex-column">
                ...
                @if (Auth::check())
                    @if (count(Auth::user()->wishlists) > 0)
                        <ul class=" overflow-auto c-scrollbar-light list-group list-group-flush mx-1">
                            @foreach (Auth::user()->wishlists as $key => $item)
                                @php
                                    $product = \App\Models\Product::find($item->product_id);
                                @endphp
                                @if ($product != null)
                                    <li id="wishlist_item_{{ $item->id }}"
                                        class="list-group-item border-0 hov-scale-img p-0">
                                        <span class="d-flex align-items-center">
                                            ...
                                            <button class="button-not-button"
                                                onclick="removeItemFromWishlist({{ $item->id }})"><i
            ...
            <div class="w-100 px-4">
                <a href="{{ route('wishlists.index') }}"
                    ...
                    <span>{{ translate('Wishlist') }}</span> </a>
            </div>
        </div>
    </div>
</div>
