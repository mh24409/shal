...
@section('panel_content')
    ...
    @if (count($wishlists) > 0)
        <div class="row p-3  row-cols-xxl-5 row-cols-xl-4 row-cols-lg-4 row-cols-md-3 row-cols-sm-2 row-cols-2 gutters-16  mx-1 mx-md-0 mb-4">
            @foreach($wishlists as $key => $wishlist)
                @if ($wishlist->product != null)
                     <a href="javascript:void(0)" onclick="showAddToCartModal({{ $wishlist->product->id }})" class="aiz-card-box col py-3 mb-4 text-center border-right border-bottom has-transition hov-shadow-out z-1" id="wishlist_{{ $wishlist->id }}">
                        <div class="position-relative h-140px h-md-200px img-fit overflow-hidden mb-3">
                            <img src="{{ uploaded_asset($wishlist->product->thumbnail_img) }}" class="h-100"
