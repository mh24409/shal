...
@section('content')
    <section class="mb-4 pt-4">
        @if (get_setting('shop_banner') != null && get_setting('shop_banner_small') != null)
            <div class="container p-0 position-relative  d-flex justify-content-around align-items-center">
                <img class="w-100 d-none d-lg-block" src="{{ uploaded_asset(get_setting('shop_banner')) }}" alt="">
                <img class="w-100 d-bloack d-lg-none" src="{{ uploaded_asset(get_setting('shop_banner_small')) }}"
                    alt="">
            </div>
        @endif
        ...
        <section class="container d-none">
            @if (count(json_decode(get_setting('home_categories'), true)) > 0)
                <div class="aiz-carousel categories-owl-carousel" data-items="5" data-xl-items="5" data-lg-items="4"
                    data-md-items="2" data-sm-items="1" data-xs-items="1" data-arrows='true' data-infinite='false'>
                    @if (get_setting('home_categories') != null)
                        @foreach (json_decode(get_setting('home_categories'), true) as $key => $value)
                            ...
                            <a href="{{ route('products.category', $category->slug) }}"
                                class="item d-flex justify-content-center align-items-center flex-column md-gap">
                                <div class="cat-img">
                                    <div class="cat-item-hover">
                                        <svg style="color: white;transform: rotate(-45deg);"
                                ...
                                <div class="cat-name"> <strong> {{ $category->getTranslation('name') }} </strong> </div>
                            </a>
                        @endforeach
                    @endif
                </div>
        ...
        <div class="container sm-px-0 pt-2" style="margin-top: 50px;overflow: visible !important;">
            ...
            <div class="row ">
                <div class="col-md-6 d-flex align-items-center sm-gap">
                    <div class="d-none d-lg-block">
                        <button class="button-not-button" id="close_sidebar">
                            {{ translate('filter') }}
                            <i class="fa-solid fa-minus filter_plus_minus_icon"></i>
                        </button>
                    </div>
                    ...
                    <ul style="height: 50px;" class="breadcrumb align-items-center bg-transparent py-0 px-1 mb-0">
                        <li class="breadcrumb-item opacity-50">
                            <a class="text-reset" href="{{ route('home') }}">{{ translate('Home') }}</a>
                        </li>
                        @if (isset($brand_id))
                            <li class="breadcrumb-item opacity-50">
                                <a class="text-reset" href="{{ route('search') }}">{{ translate('All Brands') }}</a>
                            </li>
                            <li class="text-dark fw-600 breadcrumb-item">
                                <a class="text-reset"
                                    href="{{ route('products.brand', \App\Models\Brand::find($brand_id)->slug) }}">{{ \App\Models\Brand::find($brand_id)->getTranslation('name') }}</a>
                            </li>
                        @else
                            @if (isset($category_id))
                                <li class="breadcrumb-item opacity-50">
                            ...
                            @else
                                <li class="breadcrumb-item opacity-50">
                                    <a class="text-reset" href="{{ route('search') }}">{{ translate('Shop') }}</a>
                                </li>
                            @endif
                        @endif
                ...
                <div class="col-md-6 d-none d-lg-flex justify-content-end align-items-center">
                    <div class="d-flex align-items-center h5 fs-14 fw-700 text-capitalize">
                        {{ translate('Showing') }}
                        {{ $products->firstItem()  .'–'.' '. $products->lastItem().' '. translate('Of')  .' '. $products->total() .' '. translate('Results') }} </div>
                </div>
                <div class="col-12" >
                    @if(isset($category_id))
                       <div>
                           <h2>
                                <a class="text-reset h-5"
                                    href="{{ route('products.category', \App\Models\Category::find($category_id)->slug) }}">{{ \App\Models\Category::find($category_id)->getTranslation('name') }}</a>
                           </h2>
                        </div>
                        <h5>
                            {!! \App\Models\Category::find($category_id)->getTranslation('long_slug') !!}
                        </h5>
                    @endif
                </div>
                                        ...
                                        <ul class="p-3 mb-0 list-unstyled">
                                            ...
                                            {{-- @else
                                                ...
                                                @if (\App\Models\Category::find($category_id)->parent_id != 0)
                                                    <li class="mb-3">
                                                        <a class="text-reset fs-14 fw-600 hov-text-primary"
                                                            href="{{ route('search', \App\Models\Category::find(\App\Models\Category::find($category_id)->parent_id)->slug) }}">
                                                            <i class="las la-angle-left"></i>
                                                ...
                                                <li class="mb-3">
                                                    <a class="text-reset fs-14 fw-600 hov-text-primary"
                                                        href="{{ route('search', \App\Models\Category::find($category_id)->slug) }}">
                                                        <i class="las la-angle-left"></i>
                                                        {{ \App\Models\Category::find($category_id)->getTranslation('name') }}
                                                    </a>
                                                </li>
                                                @foreach (\App\Utility\CategoryUtility::get_immediate_children_ids($category_id) as $key => $id)
                                                    <li class="ml-4 mb-3">
                                                        <a class="text-reset fs-14 hov-text-primary"
                                                            href="{{ route('search', \App\Models\Category::find($id)->slug) }}">{{ \App\Models\Category::find($id)->getTranslation('name') }}</a>
                                                    </li>
                                                @endforeach
                                            @endif --}}
    ...
    <div style="display: none" id="getDataLoaingGif">
        <div class="d-flex align-items-center justify-content-center w-100 h-100">
            <img width="300px" src="{{ static_asset('assets/img/getDataLoding.gif') }}" alt="">
        </div>
    </div>
@endsection
